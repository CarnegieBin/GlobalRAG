...
class RayPPOTrainer(object):
    ...
    def compute_pass_k(self, data_source_reward, n_val):
        ...
        n_val: 最大k值（必须是2的指数：1,2,4,8,...）
        """

        # 所有需要计算的 k：1,2,4,..., n_val
        ks = []
        k = 1
        while k <= n_val:
            ks.append(k)
        ...
        results = {}  # 最终结果：data_source -> {k: pass@k}
        ...
        for data_source, uid_dict in data_source_reward.items():
            ...
            for k in ks:
                ...
                for uid, rewards in uid_dict.items():
                    # 取前 k 条，防止越界
                    subset = rewards[:k]

                    # 判断本组是否正确（只要有奖励 > 0）
                    if any(r > 0 for r in subset):
                        correct_groups += 1
    ...
    def _validate(self):
        ...
        if not self.config.do_search:
            for test_data in self.val_dataloader:
                ...
                if self.config.get("n_val", 1) > 1:
                    test_batch = test_batch.repeat(repeat_times=self.config.n_val, interleave=True)
        ...
        for i in range(reward_tensor.shape[0]):
            ...
            if uid not in data_source_reward[data_source]:
                data_source_reward[data_source][uid] = []

            data_source_reward[data_source][uid].append(reward)
            ...
            for batch_dict in self.train_dataloader:
                ...
                # with _timer('step', timing_raw):
                    ...
                    with _timer('adv', timing_raw):
                        ...
                        reward_tensor = self.reward_fn(batch, self.global_steps)
                    ...
                    if self.config.trainer.save_freq > 0 and self.global_steps > 50 and \
