...
from verl.trainer.ppo.ray_trainer_plan_diversity import RayPPOTrainer
import re
import numpy as np
...
def _select_rm_score_fn(data_source):
    if data_source in ['2WikiMultihopQA', 'Multihop-RAG','nq', 'triviaqa', 'popqa', 'hotpotqa', '2wikimultihopqa', 'musique', 'bamboogle', 'multi_hop_qa']:
...
class RewardManager():
    ...
    def __init__(self, tokenizer, num_examine, e5_model_path, format_score=0., validation=False) -> None:
        ...
        print(f"PSE_Evaluator initialized successfully. e5_model_path: {e5_model_path}")
    ...
    def __call__(self, data: DataProto, step: int = 0):
        ...
        for i in range(len(data)):
            ...
            if self.validation:  # 评估的时候，只关注Answer正确与否，避免计算错误
...
def main_task(config):
    ...
    reward_fn = RewardManager(tokenizer=tokenizer, num_examine=0, e5_model_path=config.e5_model_path)

    # Note that we always use function-based RM for validation
    val_reward_fn = RewardManager(tokenizer=tokenizer, num_examine=1, e5_model_path=config.e5_model_path, validation=True)
...
def test_reward_func():
    """" test reward func """
    config = PSE_Config(config_dict={'e5_model_path': './e5'})
    # 初始化PSE评估器
    solution_str = """
...
<answer> Bob Kerr and Norm Hiscock </answer>
    ...
    _evaluator = PseScore(config)
    ground_truth = {"target": ["Bob Kerr and Norm Hiscock"]}
    meta_data = {
        'plan': {
            "Q1": np.array(["Who created the Money in the Bank ladder match?", "<A1>"]),
            "Q2": ["Which film was directed by <A1>?", "<A2>"],
            "Q3": ["Who was the screenwriter of <A2>?", "<A3>"],
            # "Q4": ["Who was the screenwriter of <A2>?", "<A3>"]
         },
        'graph': [{
            'Q1': {'answer': 'Chris Jericho'},
            'Q2': {'answer': "But I'm Chris Jericho!"},
            'Q3': {'answer': "Bob Kerr and Norm Hiscock"},
            # 'Q4': {'answer': "Bob Kerr and Norm Hiscock"}
        }]
    }
    print('golden_plan', meta_data['plan'])
    print('golden_graph', meta_data['graph'])
    reward = qa_em.compute_score_em(solution_str, ground_truth, meta_data, _evaluator, step=50)
    print(reward)
